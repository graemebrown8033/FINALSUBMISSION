...
namespace speechRec
{
    public class Class1
    {
        ...
        public string[] buildArr = new string[8];
        public string[] userArr = new string[5];
        ...
        public void getUsers(int uname)
        {
            RowSet results = mySession.Execute("Select * from hr.users where id=" + uname);
           ...
           // Console.WriteLine(String.Format("{0,-3}\n{1,-10}\n{2,-10}\n{3,-10}\n{4,-10}\n", "id", "fname", "lname", "accesstype", "password"));
            ...
            foreach (Row row in results.GetRows())
            {
                ...
                userArr[0] = Convert.ToString(row.GetValue<int>("id"));
                userArr[1] = row.GetValue<String>("fname");
                userArr[2] = row.GetValue<String>("lname");
                userArr[3] = row.GetValue<String>("accesstype");
                userArr[4] = row.GetValue<String>("password");
        ...
        public void AddBuilding(int id, string build, int sensor, string locat, string info1, string info2, string info3, string info4)
        {
                mySession.Execute("INSERT into hr.building " + 
                "(id, buildName, sensor, location, info1, info2, info3, info4)" +
                "VALUES(" + id + ",'" + build + "'," + sensor + ",'" + locat + "','" + info1 + "','" + info2 + "','"+ info3 + "','" + info4 + "');");
         
        }

        public void deleteBuild(int num)
        {
            mySession.Execute("Delete from hr.building WHERE id = " + num + ";");
        }


        public int getIdBuilding()
        {
           RowSet idResult = mySession.Execute("Select * from hr.building;");

           int count = 1;

            foreach(Row row in idResult.GetRows())
            {
                count++;
            }
        ...
        public void viewAllBuild()
        {
            RowSet resultAll = mySession.Execute("Select * from hr.building;");

            foreach (Row row in resultAll.GetRows())
            {
                buildArr[0] = Convert.ToString(row.GetValue<int>("id"));
                buildArr[1] = row.GetValue<String>("buildname");
                buildArr[2] = Convert.ToString(row.GetValue<int>("sensor"));
                buildArr[3] = row.GetValue<String>("location");
                buildArr[4] = row.GetValue<String>("info1");
                buildArr[5] = row.GetValue<String>("info2");
                buildArr[6] = row.GetValue<String>("info3");
                buildArr[7] = row.GetValue<String>("info4");
            }


        }

        public void editPass(String pass, int num)
        {
            mySession.Execute("UPDATE hr.users SET password = '" + pass + "' WHERE id = " + num + ";");
        }

        public void editBuild(int id, string build, string locat, string info1, string info2, string info3, string info4)
        {
                     
            mySession.Execute("UPDATE hr.building " +
                "set buildName = "+ "'" + build + "'" +
                  ", location = "+ "'" + locat + "'" +
                  ", info1 ="+ "'" + info1 + "'" +
                  ", info2 ="+ "'" + info2 + "'" +
